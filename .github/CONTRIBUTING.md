...
After the build completes, pnpm links packages across the project for development. This means that if you run any of the projects in the `examples/` directory, they will use the local version of `@nickstracke/ponder-core`.
...
2. Create a changeset using `pnpm changeset`. The changesets CLI will ask you which package is affected (`@nickstracke/ponder-core` or `create-ponder-fork`) and if the change is a patch, minor, or major release.
